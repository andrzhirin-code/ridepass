...
def fill_order_template(data: dict) -> str:
    ...
    # 2. Превращаем форму в плоский PDF (убираем интерактивность)
    flattened_bytes = doc.convert_to_pdf()
    doc.close()

    # 3. Открываем плоский PDF для вставки QR-кода
    clean_doc = fitz.open("pdf", flattened_bytes)
    ...
    clean_page.insert_image(qr_rect, stream=qr_bytes)
