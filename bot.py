...
async def process_year(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        ...
        await message.answer("⚙️ *5. Выберите мощность двигателя:*", reply_markup=kb, parse_mode="Markdown")
...
async def process_power(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        ...
        await message.answer("✏️ Введите мощность в Ваттах:", reply_markup=back_button)
    else:
        ...
        await message.answer("🔢 *6. Введите идентификационный номер:*", reply_markup=kb, parse_mode="Markdown")
...
async def process_serial(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        ...
        await message.answer("👤 *7. Введите ваше ФИО полностью:*", reply_markup=back_button, parse_mode="Markdown")
...
async def process_phone(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        ...
        await message.answer("💨 *11. Введите максимальную скорость (км/ч):*", reply_markup=back_button, parse_mode="Markdown")
...
async def referral(message: types.Message):
    ...
    if user:
        text = (
            ...
            f"🎯 *Ставка:* 20%\n\n"
            f"🔗 *Ваша ссылка:* `{user[1]}`"
        )
        await message.answer(text, reply_markup=main_menu, parse_mode="Markdown")

@dp.message(F.text == "📞 Связь с поддержкой")
async def support(message: types.Message):
    await message.answer("📞 *Поддержка:* @ridepass_support", reply_markup=main_menu, parse_mode="Markdown")
